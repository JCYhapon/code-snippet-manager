      ...
      function addSnippet(e) {
        ...
        const language = document.querySelector("#snippetLanguage").value.trim() || "Code";
        const tags = document.querySelector("#snippetTags").value.trim();
        const description = document.querySelector("#snippetDescription").value.trim();
      ...
      function renderSnippet(snippet) {
        ...
        const tagsHTML = snippet.tags.length > 0 
          ? `<div style="display: flex; gap: 5px; flex-wrap: wrap; margin-top: 10px;">${snippet.tags.map(tag => `<span style="background: #e0e0e0; padding: 3px 10px; border-radius: 12px; font-size: 11px;">${tag}</span>`).join('')}</div>`
          : '';
        ...
        div.innerHTML = `
          ...
          ${snippet.description ? `<p style="margin: 10px 0 0; color: #666; font-size: 13px;">${snippet.description}</p>` : ''}
          ${tagsHTML}
          <button onclick="deleteSnippet(${snippet.id})" style="margin-top: 10px; background: #d32f2f; padding: 8px 16px; font-size: 12px;">Delete</button>
        `;
      ...
      function updateTagsList() {
        if (uniqueTags.size === 0) {
          listTagsEl.innerHTML = '<p style="color: #999; font-size: 12px; margin: 0;">No tags yet</p>';
          return;
        }
